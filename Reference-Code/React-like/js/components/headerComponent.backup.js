...
class HeaderComponent {
  ...
  renderNetWeight() {
    ...
    return `
      <div style="position: absolute; left: 870px; top: 330px; font-size: 50px; color: black; margin: 0; text-align: left; white-space: nowrap;">
        ${formattedWeight}
      </div>
    `;
  }
  ...
  renderTitle1() {
    ...
    // Dynamic font sizing based on unit
    let fontSize;
    if (unit === 'HDPE') {
      fontSize = 60;
    } else if (unit === 'PP') {
    ...
    // Center at 25% of page width (left section)
    // Using left: 0, width: 50%, text-align: center
    console.log('🎯 Rendering title1 with width: 50%');
    return `
      <div style="
        position: absolute; 
        left: 0; 
  ...
  renderTitle2() {
    ...
    // Dynamic font sizing and positioning based on unit
    let fontSize, top, left;
    if (unit === 'HDPE') {
      fontSize = 30; // HIGH DENSITY POLYETHYLENE
      top = 200;
      left = 35;
    } else if (unit === 'PP') {
      fontSize = 35; // POLYPROPYLENE (shorter)
      top = 200;
      left = 135;
    } else if (unit === 'PPC') {
      fontSize = 28; // POLYPROPYLENE COMPOUND (longer)
      top = 200;
      left = 135;
    } else {
      fontSize = 30;
    ...
    console.log(`
╔══════════════════════════════════════════════════════════╗
║   📝  TITLE2 RENDERING - headerComponent.js Version 2   ║
╠══════════════════════════════════════════════════════════╣
║  📦 Unit: ${unit.padEnd(48)} ║
║  📏 Font Size: ${fontSize}px${' '.repeat(42 - fontSize.toString().length)} ║
║  📍 Position: top=${top}px, left=${left}px${' '.repeat(28 - top.toString().length - left.toString().length)} ║
║  📄 Text: ${title2.substring(0, 44).padEnd(44)} ║
╚══════════════════════════════════════════════════════════╝
    `);
